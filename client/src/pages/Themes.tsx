/*
  ...
  Active theme gets gold border and checkmark.
*/
...
export default function Themes() {
  const { currentSite, saveSiteField, refreshHtml, loading } = useSite();
  ...
  const handleApply = useCallback(async (themeId: string) => {
    setApplying(themeId);
    try {
      const ok = await saveSiteField("theme", themeId);
      if (ok) {
    ...
    <div className="max-w-4xl space-y-6">
      ...
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-0.5">
          Choose a visual theme for your site. Current: <span className="text-gold font-medium">{THEMES.find((t) => t.id === currentTheme)?.name || currentTheme}</span>
        </p>
      </div>
            ...
            >
              ...
              >
                ...
                <div className="absolute top-3 left-3">
                  <div className="text-xs font-bold" style={{ color: theme.colors.accent }}>
                    ARTIST NAME
                  </div>
                  <div className="text-[10px] mt-0.5" style={{ color: theme.colors.text }}>
                    Tattoo Studio
                  </div>
                </div>
              </div>
