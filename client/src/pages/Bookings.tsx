...
export default function Bookings() {
  ...
  const loadGoogleStatus = useCallback(async () => {
    try {
      const res = await authFetch("/api/dashboard/google/status");
      if (res.ok) {
        const data = await res.json();
  ...
  const handleGoogleConnect = useCallback(async () => {
    ...
    try {
      const res = await authFetch("/api/dashboard/google/auth-url");
      if (!res.ok) throw new Error(await res.text());
      const data = await res.json();
  ...
  const handleGoogleDisconnect = useCallback(async () => {
    setGoogleLoading(true);
    try {
      const res = await authFetch("/api/dashboard/google/disconnect", { method: "POST" });
      if (!res.ok) throw new Error(await res.text());
  ...
  useEffect(() => { if (tab === "settings") { loadSettings(); loadEmailPreference(); loadGoogleStatus(); } }, [tab, loadSettings, loadEmailPreference, loadGoogleStatus]);
          ...
          {settingsLoading ? (
            ...
            <>
              ...
              <div className="space-y-4 border-t border-border pt-6">
                ...
                <p className="text-xs text-muted-foreground">
                  Sync your bookings to Google Calendar. New bookings will automatically appear on your calendar.
                </p>
                {googleConnected ? (
                  <div className="flex items-center justify-between gap-3 bg-[oklch(0.16_0.005_250)] border border-border rounded-lg p-4">
                    <div className="flex items-center gap-3">
                  ...
                  <button
                    ...
                    disabled={googleLoading}
                    className="flex items-center gap-2 px-4 py-2.5 text-sm font-medium rounded-md border border-border hover:border-gold/30 bg-[oklch(0.16_0.005_250)] hover:bg-[oklch(0.18_0.005_250)] text-foreground transition-colors disabled:opacity-50"
                  >
                    {googleLoading ? (
